atom_feed :root_url => root_url do |feed|
  ...
  @changes.each do |change|
    ...
    feed.entry change, :url => todo_item_url(item) + "##{change.id}" do |entry|
      entry.title("changed ##{item.id}: #{item.title}")
      status = (change.status.blank? ? 'comment' : change.status.split(/\n+/).join(', '))
      content = ''
      ...
      content = "<div><b>List:</b> #{h(item.project.name)}: #{h(item.list.name)}</div>" +
        "<div><b>Changes:</b> #{status}</div>" +
        content
